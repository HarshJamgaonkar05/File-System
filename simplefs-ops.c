...
int simplefs_create(char *filename){
	...
	*/
    if (strlen(filename) == 0 || strlen(filename) >= MAX_NAME_STRLEN) {
        return -1;
    }
...
int simplefs_open(char *filename){
	...
	*/
    ...
    file_handle_array[handle].inode_number = inode_num;
...
int simplefs_write(int file_handle, char *buf, int nbytes){
	...
	*/
    ...
    int allocated_blocks[MAX_FILE_SIZE];
    int num_allocated = 0;
    for (int i = 0; i < MAX_FILE_SIZE; i++) {
        ...
        if (inode.direct_blocks[i] == -1) {
            ...
            if (block_num == -1) {
                for (int j = 0; j < num_allocated; j++) {
                    simplefs_freeDataBlock(allocated_blocks[j]);
                    inode.direct_blocks[start_block + j] = -1;
                }
                return -1;
            }
            inode.direct_blocks[i] = block_num;
            allocated_blocks[num_allocated++] = block_num;
    ...
    while (bytes_written < nbytes) {
        ...
        simplefs_writeDataBlock(inode.direct_blocks[block_index], block_buf);
